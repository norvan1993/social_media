  ...
  <div class="mediaBox d-none d-sm-block">
    ...
    <div class="row" v-if="descriptionInput">
      <div class="col-12">
        ...
        <button v-if="store" @click="createDescription()" class="btn btn-primary">create Description</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created: function() {
    ...
    axios
      .get("http://carmeer.com/api/photos/" + this.photoId + "/description", {
        headers: {
  ...
  methods: {
    ...
    updateDescription() {
      ...
      axios
        .post(
          "http://carmeer.com/api/descriptions/" + this.description.id,
          form,
          {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("access_token"),
